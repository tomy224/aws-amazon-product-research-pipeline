...
class DataIntegrator:
    ...
    #     """
    #     Yahoo/Rakutenデータを横展開して結合する特別メソッド
    ...
    #         logger.warning("ベースデータにJAN列がありません。Yahoo/Rakutenデータは結合できません。")
    #         print("⚠️ ベースデータにJAN列がありません。Yahoo/Rakutenデータは結合できません。")
    #         return result_df
        
    #     # JAN列に有効な値があるか確認
    #     jan_count = result_df['JAN'].notna().sum()
    #     if jan_count == 0:
    #         logger.warning("有効なJAN情報がありません。Yahoo/Rakutenデータの結合をスキップします。")
    #         print("⚠️ 有効なJAN情報がありません。Yahoo/Rakutenデータの結合をスキップします。")
    #         return result_df
    ...
    #         # 各JANごとに上位3件を取得してマージする処理
    #         jan_list = result_df['JAN'].dropna().unique()
            
    #         for jan in jan_list:
    #             # Yahoo情報の取得（上位3件）
    #             yahoo_rows = yahoo_df[yahoo_df[key_column] == jan].head(3)
    #             for i, (_, row) in enumerate(yahoo_rows.iterrows(), 1):
    #                 # プレフィックスを付けた列名でデータを追加
    #                 for col in ['価格', '送料条件', '商品URL']:
    #                     if col in row:
    #                         col_name = f'ヤフー_{col}_{i}'
    #                         result_df.loc[result_df['JAN'] == jan, col_name] = row[col]
                
    #             # Rakuten情報の取得（上位3件）
    #             rakuten_rows = rakuten_df[rakuten_df[key_column] == jan].head(3)
    ...
    #     print(f"✅ Yahoo/Rakutenデータを横展開して結合しました: 現在 {len(result_df.columns)}列")
    #     logger.info(f"Yahoo/Rakutenデータを横展開して結合しました: {len(result_df.columns)}列")
