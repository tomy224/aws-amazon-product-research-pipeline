...
def lambda_handler(event, context):
    ...
    # 一時ファイル
    tmp_output = "/tmp/yahoo_result.csv"
    ...
    results = yahoo.search_by_jan(jan_code, max_items=MAX_ITEMS)

    if not results:
        return {"statusCode": 200, "message": f"JAN {jan_code} の商品は見つかりませんでした", "jan_code": jan_code}
